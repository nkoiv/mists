...
public class BSPDungeonGenerator implements DungeonGenerator {
	...
	public TileMap generateDungeon(int xSize, int ySize) {
		return generateBSPDungeon(xSize, ySize);
	}
	
    public static TileMap generateBSPDungeon(int xSize, int ySize) {
            int totalAreas  = 20; //TODO: Make these into parameters
            int minAreaSize = 6;
            boolean treeMode = true;
            ArrayList<BSParea> BSPareas = bspDungeon(new BSPDungeonGenerator(), xSize, ySize, totalAreas, 0.3f, 0.7f, minAreaSize, treeMode);
    ...
    public static ArrayList<BSParea> bspDungeon (BSPDungeonGenerator mapGen, int xSize, int ySize, int totalAreasToCreate, float minSize, float maxSize, int absMin, boolean tree) {
            ...
            if (!tree) {
                ...
                if(Mists.debug)Mists.logger.info("Doing "+totalSplits+" splits");
    ...
    public static int[][] generateCorridorsToAdjacentRooms (int[][] intMap, ArrayList<BSParea> arealist, float doorChance) {
            ...
            for (int i = 0; i<arealist.size();i++) {
                ...
                if(Mists.debug)Mists.logger.info("Doing area "+i);
                ...
                if(Mists.debug)Mists.logger.info("Door added, drawing corridor...");
                ...
                if(Mists.debug)Mists.logger.info("Destination of the corridor: "+destinationX+", "+destinationY);
                ...
                if(Mists.debug)Mists.logger.info("Distance: "+distanceX+", "+distanceY);
                ...
                while (digging && tilesDug <= (Math.abs(distanceX)+Math.abs(distanceY)+2)) {
                    ...
                    if (corridorXPosOnMap<1 || corridorYPosOnMap<1 || corridorXPosOnMap==mapWidth-1 || corridorYPosOnMap==mapHeight-1) {
                        if(Mists.debug)Mists.logger.info("Outside map area, interrupting corridor");
                        currentTile=-1;
                        digging=false;
                    } else {
                    ...
                    if (corridorXPosOnMap==destinationX&&corridorYPosOnMap==destinationY){
                        if(Mists.debug)Mists.logger.info("Reached the destination");
                        digging=false;
                    }
                tilesDug++;
                }
    ...
    public static int[][] generateRandomCorridors (int[][] intMap, ArrayList<BSParea> arealist, float corridorChance) {
        ...
        for (BSParea area : arealist) {
            ...
            int corridorYPosOnMap=doorwayYPosOnMap;
            while (nextTile == 0) {
                if(Mists.debug)Mists.logger.info("Direction: "+direction);
                ...
                if(Mists.debug)Mists.logger.info("Corridoring at "+corridorXPosOnMap+", "+corridorYPosOnMap);
                //Exit loop if we've reached the map edges
                if (corridorXPosOnMap<1 || corridorYPosOnMap<1 || corridorXPosOnMap==mapWidth-1 || corridorYPosOnMap==mapHeight-1) {
                        if(Mists.debug)Mists.logger.info("Out of map, interrupt the corridor");
                        nextTile=-1;
                } else {
                        nextTile = intMap[corridorXPosOnMap][corridorYPosOnMap];
                        if(Mists.debug)Mists.logger.info("In the tile "+nextTile);
                }
                //If tile is clear, draw corridor on it
                if (nextTile==0) {
                        intMap[corridorXPosOnMap][corridorYPosOnMap]=FLOOR;
                        if(Mists.debug)Mists.logger.info("Drawing corridor");
    ...
    public static int[][] doorwayCleaner (int[][] intMap) {
            ...
            if(Mists.debug)Mists.logger.info("Cleaning doorways from the "+intMap.length+"x"+intMap[0].length +" map");
    ...
    public class BSParea {
        ...
        public boolean splitArea(float minSize, float maxSize, int absMin) {
                ...
                if(Mists.debug)Mists.logger.info("Splitting at position "+splitPosition);
