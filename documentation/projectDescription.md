...
Due to this, Clearance is updated only when requested. In practice this works so that the Location tags itself "maps outdated" whenever a *structure* (not creature, they're not included in clearance maps at the moment) is added or destroyed. If the pathfinder sees this flag is up, it calls for a refresh of the ClearanceMap.
